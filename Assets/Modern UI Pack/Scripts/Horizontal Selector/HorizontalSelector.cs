...
namespace Michsky.UI.ModernUIPack
{
    ...
    public class HorizontalSelector : MonoBehaviour
    {
        ...
        public void SetupItemsOnNumbers(int numberOfItems)
        {
            int startingIndex = itemList.Count;

            if (itemList.Count > numberOfItems)
            {
                int numberOfPagesToRemove = itemList.Count - numberOfItems;
                for (int i = 0; i < numberOfPagesToRemove; i++)
                {
                    RemoveItem((startingIndex - i).ToString());
                }
            }
            if (itemList.Count < numberOfItems)
            {
                int numberOfPagesToAdd = numberOfItems - itemList.Count;
                for (int i = 0; i < numberOfPagesToAdd; i++)
                {
                    CreateNewItem((startingIndex + i + 1).ToString());
                }
            }
